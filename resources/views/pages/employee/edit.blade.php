<x-admin title="Employee Create">
    <x-page-header head="Employee Create" />
    <div class="card p-3">
      <x-form method="post" action="{{ route('employee.update',$employee->id) }}">
      <div class="row">
        <div class="col-md-4">
          <x-input id="name" value="{{ $employee->name }}" required/>
        </div>
        <div class="col-md-4">
          <x-select id="district" selectedId="{{ $employee->district_id }}" :options="$districts" has-modal modal-open-id="district_model" />
        </div>
        <div class="col-md-4">
          <x-input id="contact" value="{{ $employee->contact }}" required/>
        </div>
        <div class="col-md-4">
          <x-input id="email" value="{{ $employee->email }}" type="email"/>
        </div>
        <div class="col-md-4">
          <x-input id="address" value="{{ $employee->address }}" required/>
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-4">
          <div class="form-group form-check">
            <input type="checkbox" class="form-check-input" {{ $employee->is_user == true ? 'checked':'' }} id="is_user" name="is_user">
            <label class="form-check-label" for="is_user">Is User</label>
          </div>
        </div>
        <div class="col-md-12" id="user_section"> 
          <div class="row">
            ...
            <div class="col-md-4">
              <x-input id="user_name" value="{{ $employee->user->username ?? null }}" />
            </div>
            <div class="col-md-4">
              <x-input id="password" type="password" />
            </div>
            <div class="col-md-4">
              <x-input id="cpassword" type="password" />
            </div>
            <div class="col-md-4">
              <x-select id="role" selectedId="{{ $employee->user->role->role_id ?? null }}" :options="$roles" />
            </div>
          </div>
        </div>
        <div class="col-md-12">
