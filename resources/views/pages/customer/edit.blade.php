<x-admin title="Customer Create">
    ...
    <div class="card p-3">
      <x-form method="post" action="{{ route('customer.update',$customer->id) }}">
      <div class="row">
        <div class="col-md-4">
          <x-input id="name" value="{{ $customer->name }}" required/>
        </div>
        <div class="col-md-4">
          <x-select id="district" selectedId="{{ $customer->district_id }}" :options="$districts" has-modal modal-open-id="district_model" />
        </div>
        <div class="col-md-4">
          <x-input id="contact" value="{{ $customer->contact }}" required/>
        </div>
        <div class="col-md-4">
          <x-input id="email" value="{{ $customer->email }}" type="email"/>
        </div>
        <div class="col-md-4">
          <x-input id="opening_due" value="{{ $customer->opening_due }}"/>
        </div>
        <div class="col-md-4">
          <x-input id="address" value="{{ $customer->address }}" required/>
        </div>
        <div class="col-md-4">
