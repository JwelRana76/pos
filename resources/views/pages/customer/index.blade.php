<x-admin title="Customer">
    ...
    <a href="{{route('customer.create')}}" class="btn btn-sm btn-primary m-2">
        <i class="fas fa-fw fa-plus"></i> Add Customer
    </a>
    <div class="row">
        <div class="col-md-12">
    ...
    @push('js')
        <script>
            $('#customer_payment').on('show.bs.modal', function (event) {
                var button = $(event.relatedTarget); // Button that triggered the modal
                var dataId = button.data('id');
                $('#customer_payment #customer').val(dataId).change();
            });
            function paymentdetails(Id){
                $.get('/customer/payment/details/'+Id,
                    ...
                    data.map(function(item){
                        var url ='{{route("customer.payment.delete", ":id")}}'
