<x-admin title="Sale Create">
    ...
    <div class="card p-3">
      <x-form method="post" action="{{ route('sale.store') }}">
      <div class="row">
        <div class="col-md-4">
          <x-input id="date" value="{{ date('Y-m-d') }}" type="date" required/>
        </div>
        <div class="col-md-4">
          <x-select id="customer" required :options="$customers" has-modal modal-open-id="customer_model" />
        </div>
...
@push('js')
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        // Check if sale ID is stored in session
        @if (session('sale_id'))
            // Open the popup window for invoice print
            var popupWidth = 1000;
            var popupHeight = 600;
            var left = (screen.width / 2) - (popupWidth / 2);
            var top = (screen.height / 2) - (popupHeight / 2);
            window.open("/sale/invoice/" + {{ session('sale_id') }}, "Popup", "width=" + popupWidth + ",height=" + popupHeight + ",top=" + top + ",left=" + left);

            // Clear the sale ID from session
      ...
      $(document).on('change', '#customer', function() {
          ...
          $.each(customers, function(index, item) {
              if (item.id == selectedSupplierId) {
                  ...
                  console.log('Customer found:', item); // Debugging purpose
              }
          });
      });
      ...
      function addProductToTable(item) {
          ...
          $('#find-product-table tr').each(function() {
              ...
              if (productText.includes(item.code)) {
                  ...
                  if(parseInt(stock.val()) > parseInt(qtyInput.val())){
