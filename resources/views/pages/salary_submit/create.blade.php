<x-admin title="Salary Submition">
    ...
    <div class="card p-3">
      <x-form method="post" action="{{ route('salary-submit.store') }}">
      <div class="row">
        <div class="col-md-6">
          <x-input id="date" type="month" value="{{ date('Y-m', strtotime('-1 month')) }}" max="{{ date('Y-m', strtotime('-1 month')) }}" required/>
        </div>
        <div class="col-md-6">
          <x-select id="employees" :options="$employees"/>
        </div>
        <div class="col-md-12">
          <hr>
        </div>
            ...
            <tbody id="salary-product-table">
                @foreach ($particulars as $key=>$item)
                    <tr>
                        <td>{{ ++$key }}</td>
                        <td>{{ $item->name }}</td>
                        <td><input {{ $item->is_constant == true ? 'readonly':'' }} type="text" class="form-control amount" name="amount[{{ $item->id }}]"></td>
                    </tr>
                @endforeach
                <tr>
                  <td>{{ ++$key }}</td>
                  <td>Salary Crop</td>
                  <input type="hidden" id="provident_fund" name="provident_fund">
                  <input type="hidden" id="total_pryable" name="total_pryable">
                  <td><input type="text" class="form-control crop" name="amount[crop]"></td>
                </tr>
                <tr>
                  <td>{{ ++$key }}</td>
                  <td>Note</td>
                  <td><textarea row="3" cols="10" class="form-control note" name="note"></textarea></td>
                </tr>
            </tbody>
            <tfoot>
                <tr>
