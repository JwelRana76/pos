<x-admin title="Supplier Create">
    <x-page-header head="Supplier Create" />
    <div class="card p-3">
      <x-form method="post" action="{{ route('supplier.update',$supplier->id) }}">
      <div class="row">
        <div class="col-md-4">
          <x-input id="name" value="{{ $supplier->name }}" required/>
        </div>
        <div class="col-md-4">
          <x-select id="district" selectedId="{{ $supplier->district_id }}" :options="$districts" has-modal modal-open-id="district_model" />
        </div>
        <div class="col-md-4">
          <x-input id="contact" value="{{ $supplier->contact }}" required/>
        </div>
        <div class="col-md-4">
          <x-input id="email" value="{{ $supplier->email }}" type="email"/>
        </div>
        <div class="col-md-4">
          <x-input id="opening_due" value="{{ $supplier->opening_due }}"/>
        </div>
        <div class="col-md-4">
          <x-input id="address" value="{{ $supplier->address }}" required/>
        </div>
        <div class="col-md-4">
