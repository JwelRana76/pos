<x-admin title="Purchase Create">
    ...
    <div class="card p-3">
      <x-form method="post" action="{{ route('purchase.update',$purchase->id) }}">
      <div class="row">
        <div class="col-md-4">
          <x-input id="date" value="{{ $purchase->created_at->format('Y-m-d') }}" type="date" required/>
        </div>
        <div class="col-md-4">
          ...
          <div class="card">
            <table class="table">
              ...
              <tbody id="find-product-table">
                @foreach ($purchase->product_purchase as $key=>$product_purchase)
                  <tr>
                    <td>#</td>
                    <td>{{ $product_purchase->product->name??null }} [{{ $product_purchase->product->code??null }}]</td>
                    <input type="hidden" name="product_id[]" value="{{ $product_purchase->product_id }}">
                    <td><input style="width:100px" type="number" min="1" name="qty[]" value="{{ $product_purchase->qty }}" id="quantity"></td>
                    <td><input style="width:100px" name="cost[]" id="cost" value="{{ $product_purchase->unit_cost }}"></td>
                    <td class="sub_total">{{ $product_purchase->unit_cost * $product_purchase->qty }}</td>
          ...
          <x-small-card header="Calculation Part">
              ...
              <div class="mb-3">
                  <div class="input-group">
                      ...
                      <input type="text" class="form-control" value="{{ $purchase->discount_amount }}" id="discount_amount" name="discount_amount"  placeholder="Amount">
                  </div>
              </div>
              <x-inline-input id="shipping_cost" value="{{ $purchase->shipping_cost }}" />
              <hr>
              <x-inline-input id="grand_total" readonly />
              <x-inline-input id="previous_due" value="{{ $purchase->previous_due }}" readonly />
              <hr>
              <x-inline-input id="total_payable" readonly />
              <x-inline-input id="paid_amount" value="{{ $purchase->paid }}" />
